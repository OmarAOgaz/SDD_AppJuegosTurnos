...
(Previously: `currentRoundTurnDurationSeconds = baseTurnDurationSeconds + (currentRound - 1) * roundIncrementSeconds`.)
...
(Previously: `base + (round - 1) * increment` from the frozen base each round.)
...
(Previously: reorder could affect slots and/or `turnSequence`; increment not editable in break; no `betweenRoundsEnteredAtMs`; duration used base × (round − 1).)
